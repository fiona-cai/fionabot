...
token = 'NzM2MzcyMzMwNjQ0ODk3ODkz.Xxt2LA.Cqz75dbr0bHyxTMPsex96GY6nS8'
...
async def userinfo(ctx, *, member: discord.Member=None):
	...
	eb.add_field(name="Date Joined Discord (GMT)", value=str(joined), inline=False)
...
@client.command(name='chat', pass_context=True)
async def chat(context, *args, aliases=['chatterbot']):
	mesg = ' '.join(args)
	if context.message.author.id in blacklist:
		embed = discord.Embed(title='You\'re blacklisted!', description='If you think this is a mistake, contact the owner of this server', color=cola)
		await context.message.channel.send(embed=embed)
		...
		intents.typing = True
		response = chatbot.get_response(mesg)
		await context.message.channel.send('{0}'.format(response))


@client.command(name='fact', pass_context=True)
async def fact(context, aliases=['funfact']):
	if context.message.author.id in blacklist:
		embed = discord.Embed(title='You\'re blacklisted!', description='If you think this is a mistake, contact the owner of this server', color=cola)
		await context.message.channel.send(embed=embed)
	else:
		...
		await context.message.channel.send(myline)
		print(myline)

@client.command(name='poll', pass_context=True)
async def poll(context, *args, aliases=['']):
	mesg = ' '.join(args)
	if context.message.author.id in blacklist:
		embed = discord.Embed(title='You\'re blacklisted!', description='If you think this is a mistake, contact the owner of this server', color=cola)
		await context.message.channel.send(embed=embed)
...
async def coin(context, *args):
	if context.message.author.id in blacklist:
		...
		answers = random.choices(['Heads', 'Tails'])
		...
		embe = discord.Embed(title='**My Answer:** ', description='{0}'.format(answers), color=cola)
...
@client.command(name='dmoj', pass_context=True)
async def dmoj(context, *args):
	username = ' '.join(args)
	if context.message.author.id in blacklist:
		embed = discord.Embed(title='You\'re blacklisted!',
							  description='If you think this is a mistake, contact the owner of this server', color=cola)
		...
		emed = discord.Embed(title='**{}**'.format(username), description = ("DMOJ user"), color=cola)
		emed.add_field(name='Points', value = points, inline = False)
		emed.add_field(name='Performance Points', value = performance, inline = False)
		emed.add_field(name='Rank', value = rank, inline = False)
		emed.add_field(name='Solved Problems', value = len(solved), inline = False)
		emed.add_field(name='Current Rating', value = rating, inline = False)
		emed.set_footer(text='Requested by: {0}'.format(context.message.author))
		await context.message.channel.send(embed=emed)
